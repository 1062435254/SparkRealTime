...
object CanalOdsApp extends BaseAppV1 {
    ...
    override val topic: Set[String] = Set("canal_gmall_db")
    override val initGroupId: String = "CanalOdsApp"
    ...
    override def run(streamingContext: StreamingContext, ds: DStream[String], offset: ListBuffer[OffsetRange]) = {
        ...
        ds
            .flatMap(line => {
                ...
                children.map(j => (tableName, opearte.toLowerCase(), Serialization.write(j)))
            })
            //1.对普通表重要insert和update
            //2.对order_info只考虑insert
            .filter {
